 ...
 */
@Repository
public class XingFa extends Base implements PayBase {
	...
	public void signature(Map<String, String> map, Business business, PayOrder payOrder) throws Exception {
		...
		metaSignMap.put("payType", PayTran.getCode(business.getPayType()));// WX:微信支付,ZFB:支付宝支付
		metaSignMap.put("amount", AmountUtils.changeY2F(Double.toString(payOrder.getPrice())));// 单位:分
		metaSignMap.put("goodsName", payOrder.getGoodsName());// 商品名称：20位
		...
		String reqParam = "data=" + URLEncoder.encode(param, ToolKit.CHARSET) + "&merchNo="
	...
	public Map<String, String> payCallback(HttpServletRequest request) throws Exception {
		...
		if (payOrder != null) {
			...
			if (business != null) {
				...
				logger.info("鑫发api回调：" + jsonObj.toString());
				Map<String, String> metaSignMap = new TreeMap<String, String>();
				...
				if (!jsonObj.getString("payStateCode").equals("00")) {
					logger.info("鑫发失败回调了。：" + metaSignMap.toString());
					return map;
				}
	...
	public String[] getPayType() throws Exception {
		return new String[] { "支付宝扫码", "支付宝跳转", "微信扫码", "微信跳转", "微信H5" };
	}
