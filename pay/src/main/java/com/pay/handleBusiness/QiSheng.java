 ...
 */
@Repository
public class QiSheng extends Base implements PayBase {
	...
	public void handleMain(Map<String, String> map, Business business, PayOrder payOrder) throws Exception {
		...
		map.put("channel_id", business.getUid());// 系统分配商户号
		Date date = new Date();
		long ts = date.getTime();
		String timestamp = String.valueOf(ts);
		map.put("timestamp", timestamp); // 时间戳

		map.put("pay_trench", business.getPayCode()); // 支付类型
		map.put("out_bill_num", payOrder.getGameOrderNumber());// 订单号
		map.put("amount", Double.toString(payOrder.getPrice()));// 订单金额
	...
	public void signature(Map<String, String> map, Business business, PayOrder payOrder) throws Exception {
		...
		mapv.put("channel_id", map.get("channel_id"));
		mapv.put("timestamp", map.get("timestamp"));
		mapv.put("pay_trench", map.get("pay_trench"));
		mapv.put("out_bill_num", map.get("out_bill_num"));
		mapv.put("amount", map.get("amount"));

		String sign = Encryption.md5_32(getMapParam(mapv) + business.getToken(), "");
		map.put("sign", sign);
		// 发送请求，获取返回参数
		String strReturn = ToolKit.request(business.getApiUrl(), getMapParam(map));
		JSONObject job = JSONObject.fromObject(strReturn);
		if (job.getString("response_code").equals("1001")) {
			String data = job.getString("response_data");
			JSONObject jobData = JSONObject.fromObject(data);
			String pay_url = jobData.getString("pay_url");
