...
<script language="javascript">
    ...
    function CampusController($scope, $http){
        ...
        $scope.editCampus = function(target){
            ...
            openDetailIframe('autoUpdatePage.do?id=' + campusId);
        }
        ...
        $scope.synAuto = function(id){
        	if(confirm("确认同步吗？")){
		       	$http.get('synAuto.do?id=' + id,config).success(function(data, status, headers, config){
		       		if(data.state){
        ...
        function subm(){
        	if(confirm("确认删除吗？")){
            	$http.get('deleteBank.action?id=' + $("#upperId").val()+"&code="+$("#code").val(),config).success(function(data, status, headers, config){
            		$('.popbox').hide();
                	$("#upperId").val("")
                	$("#code").val("")
            		if(data.state){
		...
		function refreshList(pageNo){
            ...
            $http.get('autoList.do?pageNo=' + pageNo +'&name='+$("#name").val(),config).success(function(data, status, headers, config){
                ...
                changePageNo();
                ZENG.msgbox._hide();
                try{if(data.ADDAUTO.state)$("#ADDAUTO").show();} catch (e) {}
            });
		}
