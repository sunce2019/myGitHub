...
<script language="javascript">
    ...
    function CampusController($scope, $http){
        ...
        $scope.editCampus = function(target){
            ...
            openDetailIframe('wechatUpdatePage.do?id=' + campusId);
        }
        ...
        function subm(){
        	if(confirm("确认删除吗？")){
            	$http.get('deleteWechat.do?id=' + $("#upperId").val()+"&code="+$("#code").val(),config).success(function(data, status, headers, config){
            		$('.popbox').hide();
                	$("#upperId").val("")
                	$("#code").val("")
            		if(data.state){
		...
		function refreshList(pageNo){
            ...
            $http.get('wechatList.do?pageNo=' + pageNo +'&cardNo='+$("#cardNo").val()+'&bankAccount='+$("#bankAccount").val()+"&bankName="+$("#bankName").val()+"&flag="+$("#flag").val()+"&customer_id="+$("#customer_id").val(),config).success(function(data, status, headers, config){
