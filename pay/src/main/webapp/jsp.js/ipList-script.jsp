...
<script language="javascript">
    ...
    function CampusController($scope, $http){
        ...
        $scope.mining = function(target){
            ...
            openDetailIframe('../user/userDetail.do?id=' + campusId);
        }
        ...
        function subm(){
        	if(confirm("确认删除吗？")){
                ...
                $http.get('deleteIp.do?id=' + $("#upperId").val()+"&code="+$("#code").val(),config).success(function(data, status, headers, config){
                	ZENG.msgbox._hide();
                	$('.popbox').hide();
                	$("#upperId").val("")
                	$("#code").val("")
		...
		function refreshList(pageNo){
            ...
            $http.get('ipList.do?pageNo=' + pageNo +'&ip='+$("#ip").val(),config).success(function(data, status, headers, config){
                ...
                changePageNo();
                ZENG.msgbox._hide();
                try{if(data.ADDIP.state)$("#ADDIP").show();} catch (e) {}
            });
		}
