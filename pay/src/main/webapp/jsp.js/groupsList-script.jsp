...
<script language="javascript">
    ...
    function CampusController($scope, $http){
        ...
        $scope.editCampus = function(target){
            ...
            openDetailIframe('groupsUpdatePage.do?id=' + campusId);
        }
        ...
        function subm(){
        	if(confirm("确认删除吗？")){
            	$http.get('deleteGroups.do?id=' + $("#upperId").val()+"&code="+$("#code").val(),config).success(function(data, status, headers, config){
            		$('.popbox').hide();
                	$("#upperId").val("")
                	$("#code").val("")
            		if(data.state){
		...
		function refreshList(pageNo){
            ...
            $http.get('groupsList.do?pageNo=' + pageNo,config).success(function(data, status, headers, config){
                ...
                changePageNo();
                ZENG.msgbox._hide();
                try{if(data.ADDGROUPS.state)$("#ADDGROUPS").show();} catch (e) {}
            });
		}
