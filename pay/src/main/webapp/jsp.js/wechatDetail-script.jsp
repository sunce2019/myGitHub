...
function CampusController($scope, $http){
    ...
    function subm(){
        ...
        fd.append("file",file)
        var url = "addWechat.do";
    	if($("#id").val()!="")url = "updateWechat.do";
        $http.post(url+'?cardNo=' + $("#cardNo").val()+'&bankAccount='+$("#bankAccount").val()+'&bankName='+$("#bankName").val()+'&flag='+$("#flag").val()+'&maxLimit='+$("#maxLimit").val()+'&remarks='+$("#remarks").val()+'&id='+$("#id").val()+"&useTime="+$("#useTime").val()+"&code="+$("#code").val()+"&tail="+$("#tail").val()+"&customerid="+$("#customerid").val(), fd, {
